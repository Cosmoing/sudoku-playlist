yp.scripts.communication = function () {
	...
	let onmessage = function (event) {
		if (event.type === "message" && yp.player) {
			if (event.data.includes("NextSong")) {
				document.getElementById("next").click();
			} else if (event.data.includes("PlayPause")) {
				document.getElementById("play-pause").click();
			} else if (event.data.includes("SetVolume")) {
				var volume = parseInt(event.data.split(' ')[1]);
